...
def main():
    size_index = 'PNG_L//9ol-Model.png'
    ...
    processed_points_image_folder = 'D://A//1 InsoleDataset//WMT//Processed_Points//' + folder_index
    ...
    with open(csv_file_path, 'w', newline='') as csvfile:
        ...
        for filename in os.listdir(points_image_folder):
            if filename.endswith('.png'):
                ...
                M_rotate = cv2.getRotationMatrix2D(rotation_center, best_angle_1, 1)  # Rotation
                rotated_points_image = cv2.warpAffine(aligned_points_image, M_rotate, (cols, rows))
                M_translate = np.float32([[1, 0, optimal_bias_1[0]], [0, 1, optimal_bias_1[1]]])  # Bias
                processed_points_image = cv2.warpAffine(rotated_points_image, M_translate, (cols, rows))
                M_rotate = cv2.getRotationMatrix2D(rotation_center, best_angle_2, 1)  # Rotation
                rotated_points_image = cv2.warpAffine(processed_points_image, M_rotate, (cols, rows))
                M_translate = np.float32([[1, 0, optimal_bias_2[0]], [0, 1, optimal_bias_2[1]]])  # Bias
                processed_points_image = cv2.warpAffine(rotated_points_image, M_translate, (cols, rows))
